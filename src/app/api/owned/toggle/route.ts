...
export async function POST(req: NextRequest) {
  try {
    ...
    if (body.action === "remove") {
      const row = db
        .prepare(
          `SELECT id FROM owned_cards WHERE user_id = ? AND scryfall_id = ? ORDER BY created_at DESC LIMIT 1`,
        )
    ...
    const qty = (
      db
        .prepare(`SELECT COUNT(*) AS n FROM owned_cards WHERE user_id = ? AND scryfall_id = ?`)
        .get(userId, sid) as { n: number }
    ).n;
